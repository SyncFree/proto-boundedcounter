...
create_last_write_wins_bucket(){
	curl -X PUT -H 'Content-Type: application/json' -d '{"props":{"allow_mult":true}}' "http://"$1":"$2"/buckets/"$3"/props"
}
   ...
   do
	  ...
	  for k in $(seq 0 $((${#servers[@]}-1)))
	  	do
			...
			cmd=$SCRIPTS_ROOT"reset-script-counter $INITIAL_VALUE $bucket $BUCKET_TYPE $DEFAULT_KEY ${SERVERS[k]} ${RIAK_PB_PORT[0]} $USER_ROOT"
			echo $cmd
